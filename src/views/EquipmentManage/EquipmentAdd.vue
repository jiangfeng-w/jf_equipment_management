<template>
    ...
    >
        ...
        >
            ...
            >
                <el-option
                    v-for="item in discipline_classifications"
                    :key="item"
                    :label="item"
                    :value="item"
                />
        ...
        >
            ...
            >
                <el-option
                    v-for="item in manage_classifications"
                    :key="item"
                    :label="item"
                    :value="item"
                />
...
<script setup>
    ...
    const addEquipmentForm = reactive({
        ...
        // 负责人邮箱
        maneger_email: store.state.userInfo.email,
    })
    // 表单验证规则
    ...
    //#region 选择器的数据
    // 生产国家
    const countrys = ['美国', '英国', '日国', '法国', '德国', '中国']
    // 生产厂商
    const manufacturers = ['卡尔蔡司显微图像有限公司', '瑞士梅特勒-托利多', 'FEI Czech Republic S.r.o']
    // 设备分类
    ...
    // 管理分类
    const manage_classifications = ['公共服务类', '教学类', '学术方向类', '科研经费购置类']
    // 所属单位
    const units = [
        '南充校区',
        '石油与天然气工程学院',
