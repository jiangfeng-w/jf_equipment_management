            ...
            >
                <template #default="scope">
                    ...
                    <el-popconfirm
                        ...
                        cancel-button-text="否"
                        title="确认取消申请吗？"
                        @confirm="repairCompleted(scope.row)"
                    >
                        <template #reference>
                            <el-button
                                type="primary"
...
<script setup>
    ...
    import ExpandInfo from '@/components/EquipmentRepair/ExpandInfo.vue'
    ...
    const getTableList = async () => {
        const res = await axios.get(`/admin/equipment/repair/${store.state.userInfo.number}`)
    ...
    const reApply = async id => {
        // console.log(id)
        try {
            const res = await axios.post('/admin/repair/reapply', { id })
            if (res.status === 201) {
    ...
    const repairCompleted = async data => {
        // console.log(id)
        try {
            const res = await axios.post('/admin/repair/repaircompleted', { id: data.id, pic: data.now_pic })
            if (res.status === 201) {
                ElMessage.success(res.data.message)
                getTableList()
            }
