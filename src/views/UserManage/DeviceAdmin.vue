...
<script setup>
    ...
    import AddDeviceAdmin from '@/components/DeviceAdmin/AddDeviceAdmin.vue'
    import EditDeviceAdmin from '@/components/DeviceAdmin/EditDeviceAdmin.vue'
    ...
    const getTableList = async () => {
        const res = await axios.post('/admin/admin/list', searchForm)
        tableList.splice(0, tableList.length, ...res.data.data)
        // console.log(tableList)
    }
    // 获取列表长度
    const getLength = async () => {
        const res = await axios.post('/admin/admin/listLength', searchForm)
    ...
    const deleteAdmins = async () => {
        ...
        // console.log(ids)
        try {
            await ElMessageBox.confirm('确认删除这几个管理员的数据吗', '系统提示', {
